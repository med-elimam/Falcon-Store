...
export async function seedCore(db: AnyDb): Promise<void> {
  ...
  /* طرق الدفع (معطلة افتراضيًا) ومناطق التوصيل (معطلة وبدون رسوم) */
  for (const pm of SEED_PAYMENT_METHODS) {
    await db
      .insert(s.paymentMethods)
      .values({ key: pm.key, labelAr: pm.labelAr, labelFr: pm.labelFr, enabled: false, sortOrder: pm.sortOrder })
      .onConflictDoNothing({ target: s.paymentMethods.key });
  }
  const existingZones = await db.select().from(s.deliveryZones);
  if (existingZones.length === 0) {
    for (const [i, nameAr] of SEED_DELIVERY_ZONES.entries()) {
      await db.insert(s.deliveryZones).values({ nameAr, enabled: false, feeMru: null, sortOrder: i });
    }
  }
  ...
  await db
    ...
    .values({
      key: "operations",
      value: { orderNotifyWhatsapp: null, defaultStockBehavior: "deduct", lowStockThreshold: 3, hoursAr: null },
    })
    .onConflictDoNothing({ target: s.siteSettings.key });
